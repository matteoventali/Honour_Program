...
def plot_training_variance(reward_histories, window_size=100, title="Training Performance Across Seeds", filename="img/training_variance.png", label="Learning reward", epsilon_histories=None, epsilon_labels=None, exploration_label="Epsilon"):
    """Plot aggregate rewards with variance and optional exploration curves."""
    ...
    if epsilon_histories is not None:
        ...
        for index, epsilon_curve in enumerate(epsilon_curves):
            if epsilon_labels is not None:
                epsilon_label = f"{exploration_label} q={epsilon_labels[index]}"
            elif len(epsilon_curves) == 1:
                epsilon_label = exploration_label
            else:
                epsilon_label = f"{exploration_label} {index + 1}"
            epsilon_axis.plot(
                episodes,
                epsilon_curve,
                color=EPSILON_COLOR,
        ...
        epsilon_axis.set_ylabel(exploration_label)
...
def plot_shaping_reward_breakdown(true_rewards, total_rewards, eps_histories, window_size=100, filename="img/shaping_reward_breakdown.png", title="Shaping Agent Reward Analysis", exploration_label="Epsilon"):
    """Plot rewards and an exploration diagnostic on independent axes."""
    if window_size <= 0:
        raise ValueError("window_size must be greater than zero")
    ...
    for index, history in enumerate(exploration_runs):
        ...
        label = exploration_label if len(exploration_runs) == 1 else f"{exploration_label} q={index} ({phase})"
        epsilon_axis.plot(
            episodes,
            history,
            color=EPSILON_COLOR,
