"""Evaluate one or more multilevel LTLf SAC-Discrete policies."""
...
def _load_actor_checkpoint(policy_path, device):
    """Load an SAC-Discrete checkpoint and its actor architecture metadata."""
    ...
    if not isinstance(checkpoint, dict) or "actor_state_dict" not in checkpoint:
        raise ValueError(
            f"{policy_path} is not a multilevel SAC-Discrete checkpoint"
        )
    return checkpoint
...
def evaluate_policy(policy, policy_dir, episodes, render, formula, waypoints_dict,
    ...
    try:
        checkpoint = _load_actor_checkpoint(policy_path, device)
        expected_state_dim = env.observation_space.shape[0] + len(automaton_states)
        if int(checkpoint["state_dim"]) != expected_state_dim:
            raise ValueError(
                f"Checkpoint expects {checkpoint['state_dim']} state features, "
                f"but this task provides {expected_state_dim}"
            )
        if int(checkpoint["action_dim"]) != env.action_space.n:
            raise ValueError("Checkpoint action dimension does not match LunarLander")
        actor = Actor(
    ...
    try:
        for episode in range(episodes):
            ...
            while not (success or terminated or truncated):
                ...
                # Deterministic SAC-Discrete evaluation chooses the modal action.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Evaluate multilevel LTLf SAC-Discrete policies for LunarLander.")
    parser.add_argument(
        "policies",
        nargs="*",
