...
def evaluate_policy_worker(policy_filename, episodes, render, waypoints, goal_reward, grid_w, grid_h):
    """
    Worker function to load and evaluate a single PyTorch DQN policy.
    Handles an N-phase sequential task with dynamic one-hot encoding.
    """
    policy_path = os.path.join("./policy/", policy_filename)
    num_phases = len(waypoints)
    extra_dims = num_phases  # The one-hot array length matches the number of phases
    
    if not os.path.exists(policy_path):
        print(f"[{policy_filename}] Error: Could not find the file '{policy_path}'")
        return policy_filename, [], []
        
    print(f"[{policy_filename}] Loading and starting evaluation ({num_phases} Phases)...")
    ...
    for ep in range(episodes):
        ...
        while not (terminated or truncated):
            ...
            # --- Dynamic State 'q' transition logic ---
            if q < num_phases - 1:
                target_x, target_y = waypoints[q]
                if abstract_x == target_x and abstract_y == target_y:
                    success_counts[q] += 1
                    print(f"[{policy_filename}] Phase {q+1}/{num_phases} Reached! Waypoint: {waypoints[q]}")
                    q += 1
            else:
                # Goal Logic (Final Phase)
                goal_x, goal_y = waypoints[-1]
                if abstract_x == goal_x and abstract_y == goal_y:
                    success_counts[q] += 1
                    step_reward = goal_reward
                    print(f"[{policy_filename}] Final Goal Reached: {waypoints[-1]}")
                    terminated = True
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate N-phase sequential DQN policies for LunarLander.")
    ...
    waypoints = [tuple(wp) for wp in config['waypoints']]
    grid_w = config.get('grid_w', 12)
    grid_h = config.get('grid_h', 12)
    goal_reward = config.get('goal_reward', 10000.0)
    
    print(f"Loaded config: {len(waypoints)} Phases. Final Goal: {waypoints[-1]}")
    ...
    if args.parallel and len(args.policies) > 1:
        ...
        with multiprocessing.Pool(processes=len(args.policies)) as pool:
            tasks = [(p, args.episodes, args.render, waypoints, goal_reward, grid_w, grid_h) for p in args.policies]
        ...
        for p in args.policies:
            name, rewards, success_counts = evaluate_policy_worker(p, args.episodes, args.render, waypoints, goal_reward, grid_w, grid_h)
            results_dict[name] = rewards
            hits_str = " | ".join([f"Phase {i+1}: {c}/{args.episodes}" for i, c in enumerate(success_counts)])
            print(f"[{name}] Eval complete. Avg Reward: {np.mean(rewards):.2f} | {hits_str}")
