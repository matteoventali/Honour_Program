...
class CircularRegion:
    ...
    @classmethod
    def from_dict(cls, data, name="region"):
...
def truth_assignment_from_observation(regions, observation):
    """Evaluate propositions exactly on continuous environment coordinates."""
...
def grid_cell_bounds(x, y, width, height):
...
def circle_intersects_cell(region, bounds):
...
def rasterize_regions(regions, width, height):
    """Over-approximate every region with all intersected grid cells."""
    rasterized = {}
...
def truth_assignment_from_cell(region_cells, x, y):
