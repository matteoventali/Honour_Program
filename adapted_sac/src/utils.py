...
def plot_shaping_reward_breakdown(true_rewards, total_rewards, exploration_history, window_size=100, filename="img/shaping_reward_breakdown.png", exploration_label="Entropy coefficient (alpha)", title="SAC Reward Analysis"):
    """Plot rewards and SAC exploration together with independent y-axes."""
    if window_size <= 0:
        raise ValueError("window_size must be greater than zero")
    true_rewards = np.asarray(true_rewards, dtype=np.float64)
    total_rewards = np.asarray(total_rewards, dtype=np.float64)
    exploration_history = np.asarray(exploration_history, dtype=np.float64)
    if (
        true_rewards.ndim != 1
        or total_rewards.shape != true_rewards.shape
        or exploration_history.shape != true_rewards.shape
    ):
        raise ValueError("reward and exploration histories must be equally sized vectors")

    _prepare_plot_path(filename)
    figure, reward_axis = plt.subplots(figsize=(7.2, 4.4), constrained_layout=True)
    exploration_axis = reward_axis.twinx()
    ...
    exploration_axis.plot(
        episodes,
        _trailing_mean(exploration_history, window_size),
        color=EPSILON_COLOR,
        linestyle=EPSILON_LINESTYLES[0],
        linewidth=1.4,
        label=exploration_label,
    )
    exploration_axis.set_ylabel(exploration_label)
    exploration_axis.grid(False)
    exploration_axis.spines["top"].set_visible(True)
    exploration_axis.spines["right"].set_visible(True)
    ...
    _place_legend_above(figure, reward_lines + exploration_lines, reward_labels + exploration_labels, max_columns=3)
