...
from shaping_diagnostics import ShapingDiagnostics
...
def _raw_training_potential(observation, q, abstract_state, abstract_mdp, mode):
    """Evaluate the configured cellwise or continuous training potential."""
    if mode == "bilinear":
        return bilinear_grid_potential(observation, q, abstract_mdp)
    return float(abstract_mdp.v_star.get(abstract_state, 0.0))
...
def _write_run_header(log_handle, episodes, use_shaping, goal_reward, abstract_mdp, automaton_states, training_shaping_gamma, shaping_frequency, normalize_training_rewards, training_potential, ground_gamma):
    ...
    reward_scale = float(goal_reward) if normalize_training_rewards else 1.0
    header = (
        "\n=== NEW RUN ===\n"
        f"episodes={episodes}, shaping={use_shaping}, goal_reward={goal_reward}, abstract_gamma={abstract_mdp.gamma}, ground_gamma={ground_gamma}\n"
        f"training_shaping_gamma={training_shaping_gamma}, shaping_formula={shaping_formula}, shaping_frequency={shaping_frequency}\n"
        f"normalize_training_rewards={normalize_training_rewards}, reward_scale={reward_scale:g}, effective_goal_reward={float(goal_reward) / reward_scale:g}\n"
        f"training_potential={training_potential}\n"
        "terminal_potential=0 for bootstrap-terminal transitions\n"
...
def run_sequential_training(env, agent, abstract_mdp, episodes, goal_reward=10000, save_policy=True, use_shaping=True, log_file=None, diagnostic_file=None, log_interval=100, training_shaping_gamma=True, shaping_frequency="event", normalize_training_rewards=False, training_potential="cell", seed=None, policy_suffix=""):
    """
    ...
    synthetic goal reward plus potential-based shaping. Shaping frequency is
    configurable between every environment step and abstract-state changes.
    """
    ...
    if shaping_frequency not in {"event", "every_step"}:
        raise ValueError("shaping_frequency must be 'event' or 'every_step'")
    if training_potential not in {"cell", "bilinear"}:
        raise ValueError("training_potential must be 'cell' or 'bilinear'")
    if normalize_training_rewards and goal_reward <= 0:
        raise ValueError("goal_reward must be positive when normalizing training rewards")
    reward_scale = float(goal_reward) if normalize_training_rewards else 1.0
    ...
    _write_run_header(log_handle, episodes, use_shaping, goal_reward, abstract_mdp, automaton_states, training_shaping_gamma, shaping_frequency, normalize_training_rewards, training_potential, agent.gamma)
    ...
    try:
        for episode in range(episodes):
            ...
            initial_x, initial_y = _abstract_position(raw_state, abstract_mdp)
            initial_abstract_state = (initial_x, initial_y, q)
            initial_raw_phi = _raw_training_potential(
                raw_state,
                q,
            ...
            episode_steps = 0
            episode_task_reward = float(goal_reward) / reward_scale if succeeded else 0.0
            ...
            while not episode_done:
                ...
                spatial_changed = (x, y) != (next_x, next_y)
                ...
                if automaton.is_goal_reached(next_q):
                    synthetic_goal_reward = float(goal_reward) / reward_scale
                    succeeded = True
                ...
                # Apply shaping at the configured environment-step frequency.
                shaping_signal = 0.0
                raw_phi_state = _raw_training_potential(
                    raw_state, q, abstract_state, abstract_mdp, training_potential
                )
                raw_phi_next = _raw_training_potential(
                    next_raw_state,
                    next_q,
                    abstract_next_state,
                    abstract_mdp,
                ...
                phi_state = raw_phi_state / reward_scale
                phi_next_state = raw_phi_next / reward_scale
                effective_phi_next = 0.0 if bootstrap_terminal else phi_next_state
                if use_shaping and (
                    shaping_frequency == "every_step"
                    or abstract_changed
                    or bootstrap_terminal
                ):
                    # Classical PBRS must use the learner's discount.  The
                    # abstract MDP may use a different, coarser time scale.
                    training_discount = agent.gamma if training_shaping_gamma else 1.0
                    shaping_signal = training_discount * effective_phi_next - phi_state

                if diagnostics:
                    diagnostics.record_transition(
                        step=episode_steps,
                        state=abstract_state,
                        next_state=abstract_next_state,
                        raw_phi_state=raw_phi_state,
                        raw_phi_next=raw_phi_next,
                        real_truth=truth_assignment,
                        cell_truth=abstract_mdp._get_truth_assignment(next_x, next_y),
                        abstract_changed=abstract_changed,
                        spatial_changed=spatial_changed,
                        q_changed=dfa_changed,
                        applied_shaping=shaping_signal,
            ...
            if diagnostics:
                diagnostics.finish_episode(succeeded, episode_shaping_reward)
...
def main(args):
    ...
    ground_steps_per_cell = float(config.get("ground_steps_per_cell", 1.0))
    if ground_steps_per_cell <= 0.0:
        raise ValueError("ground_steps_per_cell must be greater than zero")
    ground_gamma = gamma ** (1.0 / ground_steps_per_cell)
    ...
    print(
        ...
        f"Discounts: abstract={gamma:.8f}, ground={ground_gamma:.8f} "
        f"({ground_steps_per_cell:g} ground steps/cell)\n"
    ...
    if not args.post_process:
        ...
        for run_index, run_seed in enumerate(seeds, start=1):
            ...
            try:
                ...
                metrics = run_sequential_training(env=env, agent=agent, abstract_mdp=abstract_mdp, episodes=args.episodes, goal_reward=goal_reward, use_shaping=not args.no_shaping, log_file=f"{log_dir}/single_epsilon_training_seed_{run_seed}.log", diagnostic_file=f"{log_dir}/shaping_diagnostics_{args.shaping_frequency}_{args.training_potential}_{'normalized' if args.normalize_training_rewards else 'raw'}_seed_{run_seed}.jsonl", log_interval=args.log_interval, training_shaping_gamma=args.training_shaping_gamma, shaping_frequency=args.shaping_frequency, normalize_training_rewards=args.normalize_training_rewards, training_potential=args.training_potential, seed=run_seed, policy_suffix=policy_suffix)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--shaping-frequency",
        choices=["event", "every_step"],
        default="event",
        help="Apply shaping only on abstract-state changes or on every environment step.",
    )
    parser.add_argument(
        "--normalize-training-rewards",
        action="store_true",
        help="Divide both task reward and shaping potential by goal_reward.",
    )
    parser.add_argument(
        "--training-potential",
        choices=["cell", "bilinear"],
        default="cell",
        help="Use the cell value or bilinear interpolation as the learner potential.",
    )
