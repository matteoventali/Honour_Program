...
def run_sequential_training(env, agent, abstract_mdp, episodes, goal_reward=10000, save_policy=True, use_shaping=True, K=1.0, log_file=None, log_interval=100, epsilon_strategy="cascade"):
    """
...
def main(args):
    ...
    abstraction_config = AbstractionConfig.load(args.abstraction_config)
    ...
    if not args.post_process:
        ...
        try:
            ...
            # The complete training loop continues to use level 1 exactly as in
            # the original framework. Its V* now includes PBRS derived
            # recursively from every following abstraction level.
            abstract_mdp = multilevel_mdp.primary_mdp

            # Initialize one DDQN agent; the training loop maintains one
            # exploration schedule per DFA state.
            ...
            metrics = run_sequential_training(env=env, agent=agent, abstract_mdp=abstract_mdp, episodes=args.episodes, goal_reward=goal_reward, use_shaping=not args.no_shaping, K=args.shaping_scale, log_file=f"{log_dir}/multi_epsilon_training.log", log_interval=args.log_interval, epsilon_strategy=args.epsilon_strategy)
            save_training_data(f"{data_dir}/multi_epsilon_data.npz", **metrics)
        finally:
            # Release environment resources even if training fails.
            env.close()

    # Load saved metrics and generate the final diagnostic plots.
    data = np.load(f"{data_dir}/multi_epsilon_data.npz", allow_pickle=False)
    plot_buffer_fractions(data["buffer_histories"], filename=f"{plot_dir}/buffer_fractions_multi_epsilon.png", window_size=args.plot_window, state_labels=data["automaton_states"])
    plot_shaping_reward_breakdown(data["task_rewards"], data["learning_rewards"], data["epsilon_history"], window_size=args.plot_window, filename=f"{plot_dir}/reward_breakdown_multi_epsilon.png")
    print("\nFinished.")
