...
def _place_legend_above(figure, handles, labels, max_columns=4):
    ...
    # Keep the axes rectangle at the same physical size. Only the canvas area
    # reserved for additional legend rows grows.
    figure.set_layout_engine(None)
    figure.set_size_inches(base_width, total_height, forward=True)
    figure.subplots_adjust(
        left=0.11,
        right=0.89,
        bottom=0.14 * base_height / total_height,
        top=0.82 * base_height / total_height,
    )
    figure.legend(
        handles,
        labels,
        loc="lower center",
        bbox_to_anchor=(0.5, 0.835 * base_height / total_height),
        ncol=columns,
        frameon=False,
    )
